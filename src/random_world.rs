...
pub fn random_world() -> HittableList {
    ...
    let material_ground: Arc<dyn Material> = Arc::new(LambertianMaterial::new(Colour::new(0.5, 0.5, 0.5)));
        ...
        for b in -11..11 {
            ...
            if (centre - Vec3::new(4.0, 0.2, 0.0)).length() > 0.9 {
                if choose_mat < 0.8 {
                    ...
                    let sphere_material: Arc<dyn Material> = Arc::new(MetalMaterial::new(albedo, fuzz));
                    world.objects.push(
                        HittableObject::Sphere(
                    ...
                    let sphere_material: Arc<dyn Material> = Arc::new(DialectricMaterial::new(1.5));
                    world.objects.push(
                        HittableObject::Sphere(
                            Sphere::new(centre, 0.2, Arc::clone(&sphere_material))
                        )
    ...
    let material1: Arc<dyn Material> = Arc::new(DialectricMaterial::new(1.5));
    world.objects.push(
        HittableObject::Sphere(
    ...
    let material2: Arc<dyn Material> = Arc::new(LambertianMaterial::new(Colour::new(0.4, 0.2, 0.1)));
    world.objects.push(
        HittableObject::Sphere(
    ...
    let material3: Arc<dyn Material> = Arc::new(MetalMaterial::new(Colour::new(0.7, 0.6, 0.5), 0.0));
    world.objects.push(
        HittableObject::Sphere(
